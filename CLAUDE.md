...
- **MVP attributes are self-reported and PUBLIC.** Height, body silhouette, and hair/skin/eye color are entered in onboarding and shared with others through posts (Eugene, 2026-06-09). These are public profile fields — not owner-only. **Weight is the privacy carve-out: owner-only/private, used only for matching (as a coarse band), and NEVER publicly displayed** (Akash's call, 2026-06-09; slight deviation from Eugene's "all public" — to confirm). Never include `weight_kg` in public profile/post read payloads. **Weight is now REQUIRED during the not-logged-in onboarding setup** — collected like the other matching fields, but still private (Akash, 2026-06-16; supersedes the earlier "optional" framing). The no-skip / no-opt-out rule applies to the **not-logged-in setup steps only**; private ≠ optional.
