...
| FR-ON.8 | P1 | **MVP** | **Self-reported complexion & coloring. [CHG]** User selects **skin tone** from the **Monk 10-tone scale** (single tap; ordinal 1–10 for proximity matching) **and an undertone** (Warm / Cool / Neutral / Not sure — **pulled into MVP 2026-06-15** for color-match accuracy; was V2), plus **hair color** and **eye color** from category lists (Appendix 11.3). Skin tone + undertone are Teaser fields; hair/eye are Stage-3 fields **required to post** (FR-ON.20). Selection aids: jaw/inner-wrist-in-natural-light tip, larger swatches + a confirm strip, wrist-vein cue for undertone. Public profile fields (C-9), used in matching. (AI auto-detection of these → V2, AI-5/6.) |
...
| FR-RC.3 | P1 | V2 | Generate a **derived color palette profile** from complexion (V2). **MVP uses categorical coloring directly — skin tone (Monk-10), undertone, hair, eye** (undertone pulled to MVP 2026-06-15, FR-ON.8). |
...
| **ColorProfile** | user_id, **skin_tone (Monk ordinal 1–10)**, **undertone (warm/cool/neutral, MVP)**, hair_color, eye_color · *(V2: derived color_palette)* | **MVP: self-reported & PUBLIC** (C-9). Skin tone = Monk-10 ordinal for proximity matching; **undertone pulled to MVP** (FR-ON.8); hair/eye categorical. |
...
- **Body silhouette (women):** Hourglass, Pear, Rectangle, Apple, Inverted Triangle. **(men):** Athletic, Trapezoid, Triangle, Oval, Rectangle. *(Self-selected from illustrated options; mirrors the V2 scan's `classifyShape` output so the two are interchangeable.)* **[CHG 2026-06-15] User-facing labels use the recognizable fashion-industry names + a plain descriptor** (research-backed 2026-06-15; the standard 5 every stylist/fit-quiz uses): **Hourglass** "balanced, defined waist" · **Pear** "fuller hips" · **Rectangle** "straight up and down" · **Apple / Round** "fuller midsection" · **Inverted Triangle** "broader shoulders" · **Not sure/skip**. Familiarity drives self-identification; the **kindness comes from framing** — optional, reassuring, and **non-prescriptive** (we match, we don't tell people how to "fix"/"flatter" their shape). Paired with refined line-art silhouettes (`docs/design.md` §7).
- **Skin tone:** **Monk 10-tone scale** — 10 swatches, single tap, ordinal **1–10** (lightest→deepest) for proximity matching. Selection aids (`docs/design.md` §5/§7): larger swatches, a "your tone · N of 10" confirm strip, and a "hold it to your jaw/inner wrist in natural light" tip. *(Exact swatch hex in `docs/design.md` §7.)*
- **Undertone [CHG 2026-06-15 — pulled to MVP]:** Warm · Cool · Neutral · Not sure. The second axis of accurate color matching (depth × undertone); presented with plain cues (gold vs silver jewelry; inner-wrist vein color). Optional/skippable.
