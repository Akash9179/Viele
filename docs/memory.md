...
- **Stage:** documentation + planning, design locked. No application code yet. MVP scope confirmed by Eugene (2026-06-09). Docs reconciled (**SRS v1.6**, MVP plan, moderation). **All 3 core flows + data-architecture + design system designed and folded** (2026-06-15). **Design locked:** `docs/design.md` + `docs/brand.md` (native SF Pro, Eugene's layout, light/warm, real photos); mockups in `~/.gstack/projects/Viele/designs/` (home-refined, onboarding [11 screens], post-profile). v1 = **all posts public**. **Next: BUILD** — scaffold Flutter + React admin, provision the isolated Supabase project (explicit go-ahead only), apply schema as first migration. Akash will give design feedback once it runs in the iOS simulator.
